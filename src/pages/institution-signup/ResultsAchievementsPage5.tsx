...
import { ArrowLeft, ArrowRight, Trophy, Award, Star, TrendingUp, Save, Plus, X, GraduationCap, Target, Shield, Upload, FileText } from 'lucide-react'
...
export default function ResultsAchievementsPage5() {
  ...
  // Handle checkbox changes for awards and achievements
  const handleCheckboxChange = (field: string, value: string, checked: boolean) => {
    handleArrayChange(field, value, checked)
  }
  ...
  const handleDocumentUpload = (files: FileList | null) => {
    if (files) {
      ...
      const newData = { 
        ...localData, 
        certificateDocuments: [...(localData.certificateDocuments || []), ...newDocs] 
      }
      setLocalData(newData)
      updateResultsAchievements({ certificateDocuments: newData.certificateDocuments })
      debouncedSave(newData)
    }
  }

  const removeDocument = (index: number) => {
    const newDocs = (localData.certificateDocuments || []).filter((_, i) => i !== index)
    const newData = { ...localData, certificateDocuments: newDocs }
    setLocalData(newData)
    updateResultsAchievements({ certificateDocuments: newDocs })
    debouncedSave(newData)
  }
                      ...
                      {['Government Recognition', 'Education Board Awards', 'Quality Certifications (ISO)', 'Media Recognition', 'Industry Awards', 'Excellence Awards'].map(award => (
                        <div key={award} className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => handleCheckboxChange('institutionAwards', award, checked as boolean)}
                          />
                      ...
                      {['Sports Awards', 'Cultural Awards', 'Academic Excellence', 'Competition Winners', 'Science Olympiads', 'Debate Competitions', 'Art & Music', 'Leadership Awards'].map(achievement => (
                        <div key={achievement} className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => handleCheckboxChange('studentAchievements', achievement, checked as boolean)}
                          />
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label>Certificate Documents Upload *</Label>
                ...
                {(localData.certificateDocuments || []).length > 0 && (
                  <div className="space-y-2">
                    <Label>Uploaded Documents:</Label>
                    <div className="space-y-2">
                      {localData.certificateDocuments.map((doc, index) => (

...
export class StatsService {
  ...
  /**
   * Get count of partner institutions (verified institutions)
   */
  ...
  private static async getSuccessRate(): Promise<number> {
    try {
      ...
      const [verifiedTutors, verifiedInstitutions] = await Promise.all([
        supabase
          .from('tutor_profiles')
          .select('*', { count: 'exact', head: true })
          .eq('verified', true),
      ...
      const totalVerified = (verifiedTutors.count || 0) + (verifiedInstitutions.count || 0);
